...
#DEEP_LINK_URL="http://WIN-FKL67IRSIPI:9080/controller/#location=APP_EVENT_VIEWER_MODAL&eventSummary="
#./bppm-alert.sh "${APP_NAME}" "${APP_ID}" "${EN_TIME}" "${PRIORITY}" "${SEVERITY}" "${TAG}" "${EN_NAME}" "${EN_ID}" "${EN_INTERVAL_IN_MINUTES}" "${NUMBER_OF_EVENT_TYPES}" "${EVENT_TYPE_1}" "${EVENT_TYPE_1_NUM}" "${NUMBER_OF_EVENT_SUMMARIES}" "${EVENT_SUMMARY_1_ID}" "${EVENT_SUMMARY_1_TIME}" "${EVENT_SUMMARY_1_TYPE}" "${EVENT_SUMMARY_1_SEVERITY}" "${EVENT_SUMMARY_1_STRING}" "{$DEEP_LINK_URL}"
